...
def cli() -> Namespace:
    ...
    parser.add_argument(
        ...
        nargs="*",
        help="""
        Directories below which any number of header files could be located.
        Mostly used by the non hermetic parts of the CC toolchain.
        Those directories are always also include paths provided to the compiler.
        """,
    )
    parser.add_argument(
        "--toolchain_files",
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        type=Path,
        help="Store all discovered headers in the specified file in json format.",
    )
    parser.add_argument(
        "--param_file",
        metavar="FILE",

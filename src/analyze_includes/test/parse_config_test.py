...
class TestGetIgnoredIncludes(unittest.TestCase):
    def test_no_config_and_no_system_headers_info_provided(self) -> None:
        ignored_includes = get_ignored_includes(config_file=None, system_headers_info=None)
    ...
    def test_system_headers_info_replaces_hard_coded_headers(self) -> None:
        ignored_includes = get_ignored_includes(
            config_file=None, system_headers_info=Path("src/analyze_includes/test/data/config/system_headers_info.json")
        )

        self.assertEqual(sorted(ignored_includes.paths), sorted(["foo/bar.h", "some_header"]))
        self.assertEqual(ignored_includes.patterns, [])

    def test_empty_config_file(self) -> None:
        ignored_includes = get_ignored_includes(
            config_file=Path("src/analyze_includes/test/data/config/empty.json"),
            system_headers_info=Path("src/analyze_includes/test/data/config/system_headers_info.json"),
        )
    ...
    def test_extra_ignore_paths(self) -> None:
        ...
        self.assertEqual(sorted(ignored_includes.paths), sorted(["foo/bar.h", "some_header", "foo", "bar"]))
        self.assertEqual(ignored_includes.patterns, [])
    ...
    def test_override_default_ignore_patterns(self) -> None:
        ...
        self.assertEqual(sorted(ignored_includes.paths), sorted(["foo", "bar", "foobar"]))
        self.assertEqual(ignored_includes.patterns, [])

    def test_ignore_patterns(self) -> None:
        ignored_includes = get_ignored_includes(
            config_file=Path("src/analyze_includes/test/data/config/ignore_patterns.json"), system_headers_info=None
        )

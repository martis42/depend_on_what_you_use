...
def _extract_includes_from_files(ctx, target, files, cc_toolchain):
    ...
    for file in files:
        ...
        compile_env, compile_cmd = _create_compile_cmd(ctx, target, cc_toolchain, file.path, pp_file.path)
        ...
        output = ctx.actions.declare_file("{}.dwyu_pp_output.txt".format(file.basename))
        ...
        ctx.actions.run_shell(
            ...
            outputs = [output, pp_file],
            command = cmd,
            mnemonic = "DwyuGccLikePreprocessing",
            env = compile_env,
        )

        includes_file = ctx.actions.declare_file("{}.dwyu_pp_included_headers.json".format(file.basename))
        includes_args = ctx.actions.args()
        includes_args.add("--input", output)
        includes_args.add("--file", file)
        includes_args.add("--toolchain_headers_info", ctx.attr._cc_toolchain_headers[DwyuCcToolchainHeadersInfo].headers_info)
        includes_args.add("--output", includes_file)
        ctx.actions.run(
            executable = ctx.executable._tool_preprocessor_output_parser,
            inputs = [output, ctx.attr._cc_toolchain_headers[DwyuCcToolchainHeadersInfo].headers_info],
...
def dwyu_aspect_impl(target, ctx):
    ...
    if ctx.attr._use_cc_toolchain_preprocessor:
        ...
        analysis_inputs = [processed_target] + processed_deps + processed_impl_deps + ctx.files._ignored_includes + public_includes + private_includes
        ctx.actions.run(

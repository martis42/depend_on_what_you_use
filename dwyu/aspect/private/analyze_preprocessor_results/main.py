...
from dwyu.aspect.private.analyze_preprocessor_results.lib import get_relevant_includes


def cli() -> Namespace:
    parser = ArgumentParser()
    parser.add_argument(
        "--preprocessed_public_files",
        required=True,
        metavar="FILE",
        nargs="*",
        type=Path,
        help="Preprocessor results for all public source files of the target under inspection.",
    )
    parser.add_argument(
        "--preprocessed_private_files",
        required=True,
        metavar="FILE",
        nargs="*",
        type=Path,
        help="Preprocessor results for all private source files of the target under inspection.",
    )
    parser.add_argument(
...
def main(args: Namespace) -> int:
    ...
    all_includes_from_public = get_relevant_includes(
        preprocessed_files=args.preprocessed_public_files, ignored_includes=ignored_includes
    )
    all_includes_from_private = get_relevant_includes(
        preprocessed_files=args.preprocessed_private_files, ignored_includes=ignored_includes
    )

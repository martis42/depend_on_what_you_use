...
void PrintTo(const dwyu::TargetUsage::Status& status, std::ostream* os) {
    switch (status) {
...
TEST(GetSystemUnderInspection, GivenTargetUnderInspectionReadAllDataCorrectly) {
    ...
    EXPECT_EQ(tui.header_files.size(), 2);
    EXPECT_TRUE(tui.header_files.find("some/hdr_1.h") != tui.header_files.end());
    EXPECT_TRUE(tui.header_files.find("some/hdr_2.h") != tui.header_files.end());
}

...
def cli() -> Namespace:
    parser = ArgumentParser(
        ...
        description="""
This script expects that the user has invoked DWYU in the given workspace beforehand and by doing so generated DYWU report files in the output path.
Running this script multiple times on the same report files will not break anything, but will cause a lot of warnings.
Always execute the DWYU aspect to generate fresh report files before executing this script.

Beware that there are limits on what buildozer can achieve.

...
class GatherDirectIncludesHook : public PreprocessingHooksBase {
  public:
    ...
    bool locate_include_file(ContextT& ctx,
        ...
        // whenever 'PreprocessingHooksBase::throw_exception' swallows a 'severity_error' mid-recursion (e.g.
        // 'bad_include_file' for a '<system>' header DWYU does not stage): the two callbacks stop being balanced,
        // 'include_depth_' never returns to 0, and every top-level '#include' after that point is misattributed as
        // nested and dropped. The token position stays correct regardless of swallowed exceptions. We use
        // 'boost::filesystem::equivalent' (not a string compare) because Wave reports an absolutized path for the
        // current file, while the root path arrives relative, so a naive string '==' would never match.
        boost::system::error_code error_code{};
        const bool in_root_file =
            boost::filesystem::equivalent(ctx.get_main_pos().get_file().c_str(), root_file_, error_code) && !error_code;
